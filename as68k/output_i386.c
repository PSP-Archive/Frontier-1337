...
void i386_end (const char *src_filename)
{
	...
	xout ("jumptable:\n");
#ifdef GEN_DEBUG
	xln ("cmpl	$0x%x,%%eax", prev);
	xln ("jg	bad_jmp2");
#endif
	xln ("shrl	$1,%%eax");
	/* should do bounds check XXX */
	xln ("jmp	*jtab(,%%eax,4)");
#ifdef GEN_DEBUG
	xout ("	.section	.rodata\n");
...
static enum X86REG xload_reg2 (int m68k_reg, int size, int preferred_x86_reg, int flags, const char **conv)
{
	...
	/* otherwise allocate.. */
	i = xalloc_reg (size, preferred_x86_reg);
...
static enum X86REG xload_reg (int m68k_reg, int size, int preferred_x86_reg)
{
	return xload_reg2 (m68k_reg, size, preferred_x86_reg, 0, szfull);
}
...
static enum X86REG xcopy_reg2 (int m68k_reg, int size, int preferred_x86_reg, enum RegConversion conversion)
{
	...
	copy_reg = xalloc_reg (size, preferred_x86_reg);
...
void i386_func_asl (ea_t *src, ea_t *dest, int size)
{
	...
	ifNVC { } else if (is_immediate (src) && is_reg (dest)) {
		xold_readea2 (src, 0, 0, XQUICK_IMMS);
		xold_readea2 (dest, 1, 0, XQUICK_REGS);
		xold_readea2 (src, 0, 1, XQUICK_IMMS);
